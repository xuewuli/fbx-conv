...
void Model::writeBinary(FILE *file, const fbxconv::Settings *settings)
{
...
void Mesh::writeBinary(FILE *file, const fbxconv::Settings *settings)
{
    // attribute
    attributes.writeBinary(file, settings);

    // write vertices
    if (vertices.size() > 0)
    {
        unsigned int size = vertices.size();
        if (!settings->packVertex)
        {
            write(size * 4, file);
        ...
        {
            unsigned int verticeCount = vertices.size() / vertexSize;
            unsigned int packSize = getPackVertexSize();
            write(verticeCount * packSize, file);
            char *data = (char *)alloca(packSize);

            float *v = &vertices[0];
            for (unsigned int i = 0; i < verticeCount; i++)
            {
                write(packVertex(v, data), packSize, file);
                v += vertexSize;
            }
        }
    }
...
void Attributes::writeBinary(FILE *file, const fbxconv::Settings *settings)
{
    ...
    if (settings->packVertex)
    {
        for (unsigned int i = 0; i < length(); i++)
        {
            ...
            attrib = packedAttributemap.find(key)->second;
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        write(attribs[i].normalize, file);
    }
}
